...
/**
 * instant-search.php - Typesense instant search template
 *
 * @version     3.4.0
	...
	foreach ( $args['post_types'] as $post_type ) {
		...
		if ( is_wp_error( $schemas[ $post_type ] ) ) {
			if ( current_user_can( 'administrator' ) ) {
				...
				?>
                <strong><?php echo wp_sprintf( 'There seems to be some issue with settings. Please check the logs from %1$s here %2$s or troubleshoot from %3$s here %4$s', '<a href="' . $cm_ts_settings_url . '">', '</a>', '<a href="https://docs.wptypesense.com/debug/debug/">', '</a>' ); ?></strong>
				<?php
				return;
			}
		} else {
			...
			if ( ! empty( $facets[ $schemaName ] ) ) {
				$facets[ $schemaName ] = apply_filters( 'cm_typesense_search_facets', $facets[ $schemaName ], $schemaName );
			}
		}
	}
endif;
...
<div class="cmswt-InstantSearch ais-InstantSearch <?php echo esc_html( implode( ' ', $additional_classes ) ); ?>"
     ...
     data-additional_config="<?php echo _wp_specialchars( json_encode( apply_filters( 'cm_typesense_additional_config', [] ) ), ENT_QUOTES, 'UTF-8', true ); ?>"
